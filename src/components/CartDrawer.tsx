      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 bottom-0 z-50 w-full sm:w-[440px] bg-background flex flex-col shadow-elegant"
          ...
          >
              ...
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center py-20">
                  <div className="w-20 h-20 rounded-full bg-muted flex items-center justify-center mb-6">
                    <svg className="w-8 h-8 text-muted-foreground" fill="none" stroke="currentColor" strokeWidth="1.2" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l-1 12H6L5 9z" />
                    </svg>
                  </div>
                  <p className="font-serif text-xl text-foreground mb-2">Votre sélection est vide</p>
                  ...
                  <Link
                    to="/catalogue"
                    onClick={close}
                    className="text-xs uppercase tracking-[0.2em] text-primary border-b border-primary pb-1 hover:opacity-70 transition"
                  >
                ...
                <ul className="space-y-5">
                  {items.map((item, idx) => (
                    ...
                    >
                      <div className="w-24 h-28 rounded-md overflow-hidden bg-muted flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col">
                        ...
                        <div className="mt-auto flex items-center justify-between">
                          <div className="flex items-center gap-2 border border-border rounded-full px-1 py-0.5">
                            ...
                            <span className="text-sm w-5 text-center tabular-nums text-foreground">{item.qty}</span>
                            <button
                              onClick={() => updateQty(item.productId, item.size, item.qty + 1)}
                              className="w-6 h-6 flex items-center justify-center text-muted-foreground hover:text-primary"
            ...
            {items.length > 0 && (
              <footer className="px-6 py-5 border-t border-border bg-muted/40">
                ...
                <Link
                  to="/commande"
                  onClick={close}
                  className="block w-full bg-primary text-primary-foreground text-center text-xs uppercase tracking-[0.25em] py-4 rounded-full hover:bg-primary/90 transition-colors"
                >
